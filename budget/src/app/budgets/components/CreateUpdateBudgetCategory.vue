<template>
	<tr id="budget-category-create-edit-view">
		...
		<td>
			<a class="button is-primary" @click.prevent="processSave">{{editing?'Save':'Add'}}</a>
		</td>
	</tr>
</template>
	...
	export default {
		...
		mounted() {
			...
			if (this.value) {
				...
				this.budgetCategory.category = this.getCategoryById(
		...
		methods: {
			...mapActions(["createCategory", "loadCategories"]),
			processSave() {
				if (this.editing) {
		...
		computed: {
      ...mapGetters(["getCategorySelectList","getCategoryById"])
		}
	};
</script>